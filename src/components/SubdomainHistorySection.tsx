...
import { SubdomainHistoryEntry } from '../types';
import SubdomainResultsSection from './SubdomainResultsSection';
import { Calendar, Trash2, ChevronDown, Database } from 'lucide-react';

interface SubdomainHistorySectionProps {
  history: SubdomainHistoryEntry[];
  setHistory: (history: SubdomainHistoryEntry[]) => void;
}

const SubdomainHistorySection: React.FC<SubdomainHistorySectionProps> = ({ history, setHistory }) => {
  const handleClearHistory = () => {
    if (window.confirm('Are you sure you want to clear your entire subdomain scan history? This action cannot be undone.')) {
      setHistory([]);
    }
  };
    ...
    return (
      <div className="bg-white rounded-lg shadow-md p-12 text-center border border-gray-200">
        <div className="max-w-md mx-auto">
          ...
          <p className="text-gray-600">Your past subdomain scan results will appear here.</p>
        </div>
      </div>
    );
  }
        ...
        {history.map((entry) => (
          <details key={entry.id} className="bg-white rounded-lg shadow-sm border border-gray-200 group">
            ...
            <div className="p-4 border-t border-gray-200">
              <SubdomainResultsSection results={entry.results} />
            </div>
          </details>
        ))}
      </div>
...
export default SubdomainHistorySection;
